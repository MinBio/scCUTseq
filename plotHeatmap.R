...
plotHeatmap = function(profiles, bins, order, dendrogram = TRUE, rasterize = FALSE, linesize = .5) {
  # Check that order is not specified while dendrogram is also requested
  if(!missing(order)) dendrogram = FALSE
  # Load libraries
  packages = c("data.table", "ggplot2", "cowplot", "tidyverse", "ggdendro", "ggrastr")
  ...
  if(dendrogram) {
    ...
    if(rasterize) {
      heatmap = ggplot(dt_melt) +
        ggrastr::rasterize(geom_linerange(aes(ymin = start_cum, ymax = end_cum, x = variable, color = value), size = linesize)) +
        coord_flip() +
