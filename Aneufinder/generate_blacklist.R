...
bamfile = "/mnt/AchTeraD/data/Aneufinder_refs/BICRO231_NZ86_merged_HQ_IMR90.bam"
...
lcutoff = quantile(bins$`binsize_5e+04`$counts, 0.1)
ucutoff = quantile(bins$`binsize_5e+04`$counts, 0.9985)

plt = plot(bins$`binsize_5e+04`) +
  geom_hline(aes(yintercept = lcutoff), color = "red") +
  geom_hline(aes(yintercept = ucutoff), color = "red")

blacklist = bins$`binsize_5e+04`[bins$`binsize_5e+04`$counts <= lcutoff | bins$`binsize_5e+04`$counts >= ucutoff]
