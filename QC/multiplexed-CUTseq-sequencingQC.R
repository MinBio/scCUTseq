...
for(library in libraries) {
  ...
  pre_file = list.files(paste0(top_folder, library), pattern = "all.tsv", full.names = T)
  post_file = list.files(paste0(top_folder, library), pattern = "dedup.tsv", full.names = T)
  
  pre_dedup = fread(pre_file, select = c(1, 7, 9))
  post_dedup = fread(post_file, select = 9)
...
write.table(sheet_text, paste0(top_folder, "library_overview-sheet.tsv"), quote = F, row.names = F, col.names = T, sep = "\t")
